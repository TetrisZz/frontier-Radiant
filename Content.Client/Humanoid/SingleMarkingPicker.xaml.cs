using System.Linq;
using Content.Client._radiant.Humanoid;
...
public sealed partial class SingleMarkingPicker : BoxContainer
{
    ...
    /// <summary>
    ///     Fired when hair gradient options change (hair / facial hair categories only).
    /// </summary>
    public Action? OnGradientChanged;

    private GradientHairColorPanel? _gradientPanel;
    ...
    public SingleMarkingPicker()
    {
        ...
        _gradientPanel = new GradientHairColorPanel();
        _gradientPanel.OnChanged += () => OnGradientChanged?.Invoke();
        AddChild(_gradientPanel);
    }

    public bool SupportsGradient =>
        Category is MarkingCategories.Hair or MarkingCategories.FacialHair;

    private HumanoidVisualLayers TargetLayer => Category switch
    {
    ...
    public void UpdateGradientFromAppearance(HumanoidCharacterAppearance appearance, string species)
    {
        if (_gradientPanel == null || !SupportsGradient)
        {
            if (_gradientPanel != null)
                _gradientPanel.Visible = false;
            return;
        }

        _gradientPanel.TargetLayer = TargetLayer;
        _gradientPanel.UpdateFromAppearance(appearance, species);
    }

    public void UpdateGradientFromComponent(HumanoidAppearanceComponent humanoid, string species)
    {
        if (_gradientPanel == null || !SupportsGradient)
        {
            if (_gradientPanel != null)
        ...
        var appearance = new HumanoidCharacterAppearance(
            ...
            new List<Marking>())
        {
            HairColoringMode = humanoid.HairColoringMode,
            HairGradientColor = humanoid.HairGradientColor,
            HairGradientDirection = humanoid.HairGradientDirection,
            FacialHairColoringMode = humanoid.FacialHairColoringMode,
            FacialHairGradientColor = humanoid.FacialHairGradientColor,
            FacialHairGradientDirection = humanoid.FacialHairGradientDirection,
        };

        UpdateGradientFromAppearance(appearance, species);
    }

    public HairColoringMode GetGradientMode() => _gradientPanel?.GetMode() ?? HairColoringMode.Solid;

    public Color GetGradientSecondaryColor() => _gradientPanel?.GetSecondaryColor() ?? Color.White;

    public HairGradientDirection GetGradientDirection() =>
        _gradientPanel?.GetDirection() ?? HairGradientDirection.TopToBottom;
    ...
    public void UpdateData(List<Marking> markings, string species, int totalPoints)
    {
        ...
        if (_gradientPanel != null)
            _gradientPanel.Visible = SupportsGradient;
