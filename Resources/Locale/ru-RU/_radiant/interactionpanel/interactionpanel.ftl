# Ui
interaction-ui-default-title = Взаимодействия
interaction-panel-import-button = Импорт
interaction-panel-editor = ⚙
interaction-panel-pin = Закр.
interaction-panel-unpin = Откр.
interaction-panel-constructor-button = Конструктор
interaction-panel-special-collapsible = Специальные
interaction-panel-favourites-collapsible = Избранные
interaction-panel-harmless-collapsible = Безобидные
interaction-panel-outspoken-collapsible = Откровенные
interaction-panel-lips-collapsible = Губы
interaction-panel-head-collapsible = Голова
interaction-panel-chest-collapsible = Грудь
interaction-panel-groin-collapsible = Промежность
interaction-panel-tail-collapsible = Хвост
interaction-panel-body-collapsible = Тело
interact-player = Вы
diona-leaves = ...имеет листья
diona-branches = ...имеет ветки
arachnid-nearestplayer = ...имеет что?
vox-nearestplayer = ...имеет клоаку
male-anal = ...имеет анал
male-dick = ...имеет член
female-anal = ...имеет анал
female-vagine = ...имеет вагину
female-breasts = ...имеет грудь
unsexed-nearestplayer = ...имеет что?
unknown-nearestplayer = ...имеет что?
diona-leaves-player = ...имеете листья
diona-branches-player = ...имеете ветки
arachnid-player = ...имеете что?
vox-player = ...имеете клоаку
male-anal-player = ...имеете анал
male-dick-player = ...имеете член
female-anal-player = ...имеете анал
female-vagine-player = ...имеете вагину
female-breasts-player = ...имеете грудь
unsexed-player = ...имеете что?
unknown-player = ...имеете что?
game-hud-open-interaction-menu-button-tooltip  = Открыть панель взаимодействий