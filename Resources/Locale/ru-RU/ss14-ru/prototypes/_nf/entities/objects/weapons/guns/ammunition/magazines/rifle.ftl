
ent-NFMagazineRifle20Empty = магазин (.20 винтовочный, любой)
    .suffix = пустой
    .desc = { ent-BaseMagazineRifle.desc }

ent-NFMagazineRifle20 = магазин (.20 винтовочный)
    .desc = { ent-BaseMagazineRifle.desc }

ent-NFMagazineRifle20Overpressure = магазин (.20 винтовочный усиленный)
    .desc = { ent-BaseMagazineRifle.desc }

ent-NFMagazineRifle20Incendiary = магазин (.20 винтовочный зажигательный)
    .desc = { ent-BaseMagazineRifle.desc }

ent-NFMagazineRifle20Uranium = магазин (.20 винтовочный урановый)
    .desc = { ent-BaseMagazineRifle.desc }

ent-NFMagazineRifle20Practice = магазин (.20 винтовочный тренировочный)
    .desc = { ent-BaseMagazineRifle.desc }

ent-NFMagazineBoxRifle20 = коробчатый магазин (.20 винтовочный)
    .desc = { ent-BaseMagazineRifle.desc }

ent-NFMagazineBoxRifle20Empty = коробчатый магазин (.20 винтовочный)
    .suffix = пустой
    .desc = { ent-NFMagazineBoxRifle20.desc }

ent-NFMagazineClipRifle20Empty = обойма (.20 винтовочная, любая)
    .suffix = пустая
    .desc = { ent-BaseMagazineRifle.desc }

ent-NFMagazineClipRifle20 = обойма (.20 винтовочная)
    .suffix = полная
    .desc = { ent-BaseMagazineRifle.desc }

ent-NFMagazineClipRifle20Rubber = обойма (.20 винтовочная резиновая)
    .suffix = полная
    .desc = { ent-BaseMagazineRifle.desc }

ent-NFMagazineRifle30Empty = магазин (.30 винтовочный, любой)
    .suffix = пустой
    .desc = { ent-BaseMagazineLightRifle.desc }

ent-NFMagazineRifle30 = магазин (.30 винтовочный)
    .desc = { ent-BaseMagazineLightRifle.desc }

ent-NFMagazineRifle30Overpressure = магазин (.30 винтовочный усиленный)
    .desc = { ent-BaseMagazineLightRifle.desc }

ent-NFMagazineRifle30Incendiary = магазин (.30 винтовочный зажигательный)
    .desc = { ent-BaseMagazineLightRifle.desc }

ent-NFMagazineRifle30Uranium = магазин (.30 винтовочный урановый)
    .desc = { ent-BaseMagazineLightRifle.desc }

ent-NFMagazineRifle30Practice = магазин (.30 винтовочный тренировочный)
    .desc = { ent-BaseMagazineLightRifle.desc }

ent-NFMagazineRifle30Rubber = магазин (.30 винтовочный резиновый)
    .desc = { ent-BaseMagazineLightRifle.desc }