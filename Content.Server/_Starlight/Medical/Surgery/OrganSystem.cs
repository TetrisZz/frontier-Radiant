#if false // Optional Starlight cyber-organ integration is not part of the isolated surgery port.
using System.Linq;
...
public sealed partial class OrganSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<ActionOrganComponent, SurgeryOrganImplantationCompleted>(OnActionOrganImplanted);
        SubscribeLocalEvent<ActionOrganComponent, SurgeryOrganExtracted>(OnActionOrganExtracted);
    ...
    // Radiant sector: this branch has no Starlight InherentAction component, so actions are
    // owned by the implant and explicitly follow its surgical installation lifecycle.
    private void OnActionOrganImplanted(Entity<ActionOrganComponent> ent, ref SurgeryOrganImplantationCompleted args)
    {
        _actions.AddAction(args.Body, ref ent.Comp.ActionEntity, ent.Comp.Action, ent.Owner);
        Dirty(ent);
    }

    private void OnActionOrganExtracted(Entity<ActionOrganComponent> ent, ref SurgeryOrganExtracted args)
    {
